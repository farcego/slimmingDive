...
remove dives shallower than 100 m depth and 300 s of duration. It will
sort the data by date, remove repeated rows (if any) and those with
have certain issues like more than one inflection point at the same
time.
...
drifing segment and drift rate toguether with furter calculations
depend on them.
...
attr(elek, 'update.type') <- 'kalman.single'
...
plotDrift(D2, xlab = 'foraging days',cex.lab = 1.5, cex = 2,
          ylab = 'Drift rate')
lapply(D3, function(fo){points(fo$Date, fo$pred, type = 'l', col = 'firebrick', lwd = 7)})
