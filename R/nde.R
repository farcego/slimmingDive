##' Function that calculates the drift rate of a given dive and the drift segment.
##'
##' The selection procedure is based on the 'order' of the generationof the inflection points, plus a number of criteria, that helps to determine if the dive is a drift dive, and, if so, whether is a positive or negative drift, and which is the drifting segment. The drift rate is then calculated as the difference of depths at the start and end of the dive divided by the segment duration.
##' @title Drift Rate estimator 
##' @param d a single summarized dive row with variables generated by calling function
##' @param extract flag for determining which output is requested to the function \cr
##' \itemize{
##' \item 'nde': call for retrieving the drift rate estimate
##' \item 'ds': call for retrieving the drifting segment
##' \item 'both': call for retrieving both values
##' }
##' @return a numeric value, either the drift rate or the drift segment, or a vector containing both.
