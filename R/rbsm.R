## RBSM is the standard function for now


##' Function to retrieve latent variables of the BSM (values that were generated onboard but has not been transmitted)
##'
##' This function is based on the recursively application of linear models through each summarized dive to get the order in which the inflection points have been selected by the BSM. An old function with a bug is now on file 'deprecated.R'. Note that this function,if requested, will return the smallest residual that it may or may not be the residual of the last point. This functions works at dive level, and asume some naming conventions  from the SMRU generated files. The column names requested are: D1,D2,D3,D4,T1,T2,T3,T4 and DIVE_DUR. This funcion is not intended to be used directly by an end user, but if needed to be applied directly trough a data.frame object, function rbsm should be called via apply
...
##' }
##' @return  the exact content will depended on the flag used with retrieve (see above). The returned values are stored in a vector
